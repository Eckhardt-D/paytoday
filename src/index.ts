...
class PayTodaySDK {
  config: PayConfig;
  private url: string;
  private prodURL: string = "https://paytoday.com.na/js/pay-with-paytoday.js";
  private stageURL: string = "https://dev.paytoday.com.na/js/pay-with-paytoday.js";
   ...
   */
  public createButton(element: HTMLElement, amount: number, reference: string, redirectURL: string = ''): HTMLElement {
    ...
    try {
      (window as any).createButton(this.config.businessId, this.config.businessName, amt, redirectURL, reference);
    } catch(error: any) {
 ...
 * @returns {void}
 */
export const initializePaytoday = async ({ businessId, businessName, debug=false }: PayConfig) : Promise<PayTodaySDK|unknown> => {
